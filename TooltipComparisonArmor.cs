...
namespace InformativeTooltips.Content
{
    ...
    public class MaterialLineSwap : GlobalTooltipsBase
    {
        ...
        public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
        {
            ...
            {
                TooltipLine DefLine;
                if (tooltips[3].Name != "Material") { return; }
                if (tooltips[2].Name == "Defense")
                {
                    DefLine = tooltips[2];
                    tooltips.Remove(tooltips[2]);
                    tooltips.Insert(3, DefLine);
                }
            }
        }
    }
    public class ThisIsTheOne : GlobalTooltipsBase
    {
        ...
        public void TooltipReading(Item item, List<TooltipLine> tooltips)
        {
            ...
            if (tooltips[2].Name == "Material") { z++; }
            if (!item.social) { tooltips.Insert(2 + z, SHIFTINFO);
                if (Main.keyState.IsKeyDown(Keys.LeftShift) || Main.keyState.IsKeyDown(Keys.RightShift))
                {
                    var toreplace = tooltips.IndexOf(SHIFTINFO);
                    ...
                    tooltips.Insert(toreplace, ArmorComp);
